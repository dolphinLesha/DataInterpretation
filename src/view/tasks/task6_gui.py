...
class WidgetTask6(QWidget):
    ...
    def build_graph(self):
        ...
        self.func2.build(**self.view_control.graph_and_settings2.get_settings())
        Graphic(self.view_graphics.plots["plot2"]).build(func=self.func2, prefab=GraphicPrefab.prefab_simple_thin())

        self.func3 = AddFunction()
        self.func3.build(**{'funcs': [self.func, self.func2]})
        Graphic(self.view_graphics.plots["plot3"]).build(func=self.func3, prefab=GraphicPrefab.prefab_simple_thin())

        self.anti_trend = self.func3.anti_trend()
        Graphic(self.view_graphics.plots["plot4"]).build(func=self.anti_trend,
                                                         prefab=GraphicPrefab.prefab_simple_thin())

        rand_sigma = SelfRandomAddFuncSettings()
        self.sigma_rand = RandomAddSigmaFunc()
        self.sigma_rand.build(**rand_sigma.get_values())
        Graphic(self.view_graphics.plots["plot5"]).build(func=self.sigma_rand,
                                                         prefab=GraphicPrefab.prefab_simple_thin())
...
class WidgetValues(QWidget):
    ...
    def init_ui(self):
        ...
        self.plot2 = self.build_plot_item(plot_name='plot2', plot_title='график шума')
        self.plot3 = self.build_plot_item(plot_name='plot3', plot_title='исходный с шумом')
